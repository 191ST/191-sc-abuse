...
-- Teleport dengan blink jika ada tembok
...
-- Proses memasak di setiap tahap
...
local function fullyNVLoop(setStatusFunc)
	...
	while fullyNVRunning do
		...
		if waterCount == 0 or sugarCount == 0 or gelatinCount == 0 then
			setStatusFunc("⚠️ Bahan habis! Berhenti...", C.orange)
			break
		end
...
local startNVW, startNVB = actionBtn(fullyNVPage, 11, "▶ START FULLY NV", C.greenD, C.txt)
local stopNVW, stopNVB = actionBtn(fullyNVPage, 11, "■ STOP FULLY NV", C.red, C.txt)
