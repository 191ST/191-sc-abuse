-- ELIXIR 3.5 + FULLY NV (DEEP PURPLE THEME)
...
-- ========== BLINK SYSTEM (5 studs, cooldown 1 detik) ==========
...
local function moveWithBlinkApart(targetCFrame)
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return false end
...
-- ========== BASEPLATE (ukuran 1000, kedalaman 5 studs di bawah player) ==========
local function createBasePlate()
    ...
    basePlatePart = plate
    print("✅ Baseplate dibuat di Y = " .. plateY)
    return true
end

-- ========== TELEPORT DENGAN BASEPLATE ==========
local function teleportWithBaseplate(targetPos)
    -- Turun 5 studs dulu (ke baseplate)
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if hrp then
...
local function cookAtApartment()
    ...
    -- Gerak pakai SLOW TWEEN
    for _, stage in ipairs(stages) do
        if not fullyRunning then return false end
        if fullyStatusLabel then fullyStatusLabel.Text = "→ " .. stage.name end
        moveWithBlinkApart(stage.cf)
        spamE(10)
        task.wait(0.5)
    end
    
    -- Proses masak (tanpa loading screen)
...
local function fullyNVLoop()
    while fullyRunning do
        -- 1. Teleport ke NPC beli (dengan baseplate)
        if fullyStatusLabel then fullyStatusLabel.Text = "🚀 Ke NPC Beli..." end
        showLoading("Ke NPC Beli")
        if not teleportWithBaseplate(NPC_BUY_POS) then break end
        hideLoading()
        ...
        if not teleportWithBaseplate(apartPos) then break end
        hideLoading()
...
-- ========== FULLY NV PAGE (MENGGANTIKAN PAGE STATUS) ==========
...
-- Scroll frame untuk konten Fully NV
...
apartDropdown.MouseButton1Click:Connect(function()
    ...
    menu.Position = UDim2.new(0.1, 0, 0, 115)
...
local fullyStatusLabel = Instance.new("TextLabel", statusCardNV)
...
-- Total MS Dimasak
local cookedRow = card(fullyScroll, 30, 5)
local cookedLabel = Instance.new("TextLabel", cookedRow)
cookedLabel.Size = UDim2.new(0.6, 0, 1, 0)
cookedLabel.Position = UDim2.new(0, 12, 0, 0)
...
local cookedValue = Instance.new("TextLabel", cookedRow)
cookedValue.Size = UDim2.new(0.4, -10, 1, 0)
cookedValue.Position = UDim2.new(0.6, 0, 0, 0)
cookedValue.BackgroundTransparency = 1
cookedValue.Text = "0"
cookedValue.Font = Enum.Font.GothamBold
cookedValue.TextSize = 12
cookedValue.TextColor3 = C.accentGlow
cookedValue.TextXAlignment = Enum.TextXAlignment.Right

-- Total MS Terjual
local soldRow = card(fullyScroll, 30, 6)
local soldLabel = Instance.new("TextLabel", soldRow)
soldLabel.Size = UDim2.new(0.6, 0, 1, 0)
soldLabel.Position = UDim2.new(0, 12, 0, 0)
...
waterValue.Size = UDim2.new(0.4, -10, 1, 0)
waterValue.Position = UDim2.new(0.6, 0, 0, 0)
waterValue.BackgroundTransparency = 1
waterValue.Text = "0"
waterValue.Font = Enum.Font.GothamBold
waterValue.TextSize = 11
waterValue.TextColor3 = C.accentGlow
waterValue.TextXAlignment = Enum.TextXAlignment.Right

local sugarRow = card(fullyScroll, 25, 8)
local sugarLabel = Instance.new("TextLabel", sugarRow)
sugarLabel.Size = UDim2.new(0.6, 0, 1, 0)
...
sugarLabel.TextSize = 10
sugarLabel.TextColor3 = C.textMid
sugarLabel.TextXAlignment = Enum.TextXAlignment.Left

local sugarValue = Instance.new("TextLabel", sugarRow)
sugarValue.Size = UDim2.new(0.4, -10, 1, 0)
sugarValue.Position = UDim2.new(0.6, 0, 0, 0)
sugarValue.BackgroundTransparency = 1
sugarValue.Text = "0"
sugarValue.Font = Enum.Font.GothamBold
sugarValue.TextSize = 11
sugarValue.TextColor3 = C.accentGlow
sugarValue.TextXAlignment = Enum.TextXAlignment.Right

local gelatinRow = card(fullyScroll, 25, 9)
local gelatinLabel = Instance.new("TextLabel", gelatinRow)
gelatinLabel.Size = UDim2.new(0.6, 0, 1, 0)
...
gelatinLabel.TextSize = 10
gelatinLabel.TextColor3 = C.textMid
gelatinLabel.TextXAlignment = Enum.TextXAlignment.Left

local gelatinValue = Instance.new("TextLabel", gelatinRow)
gelatinValue.Size = UDim2.new(0.4, -10, 1, 0)
gelatinValue.Position = UDim2.new(0.6, 0, 0, 0)
gelatinValue.BackgroundTransparency = 1
gelatinValue.Text = "0"
gelatinValue.Font = Enum.Font.GothamBold
gelatinValue.TextSize = 11
gelatinValue.TextColor3 = C.accentGlow
gelatinValue.TextXAlignment = Enum.TextXAlignment.Right

local emptyRow = card(fullyScroll, 25, 10)
local emptyLabel = Instance.new("TextLabel", emptyRow)
emptyLabel.Size = UDim2.new(0.6, 0, 1, 0)
...
emptyValue.Size = UDim2.new(0.4, -10, 1, 0)
emptyValue.Position = UDim2.new(0.6, 0, 0, 0)
emptyValue.BackgroundTransparency = 1
emptyValue.Text = "0"
emptyValue.Font = Enum.Font.GothamBold
emptyValue.TextSize = 11
emptyValue.TextColor3 = C.accentGlow
emptyValue.TextXAlignment = Enum.TextXAlignment.Right

-- ========== TOMBOL START/STOP ==========
local btnFrame = Instance.new("Frame", fullyScroll)
btnFrame.Size = UDim2.new(1, 0, 0, 50)
btnFrame.Position = UDim2.new(0, 0, 0, 460)
btnFrame.BackgroundTransparency = 1

local fullyStartBtn = Instance.new("TextButton", btnFrame)
...
local fullyStopBtn = Instance.new("TextButton", btnFrame)
...
fullyStartBtn.MouseButton1Click:Connect(function()
    ...
    createBasePlate() -- Buat baseplate pas START
    fullyRunning = true
...
fullyStopBtn.MouseButton1Click:Connect(function()
    ...
    -- Baseplate TIDAK dihapus (biarin aja)
end)
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            ...
            waterValue.Text = tostring(countItem("Water"))
            sugarValue.Text = tostring(countItem("Sugar Block Bag"))
            gelatinValue.Text = tostring(countItem("Gelatin"))
            emptyValue.Text = tostring(countItem("Empty Bag"))
        end)
    end
end)
