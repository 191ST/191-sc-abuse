-- ELIXIR 3.5 + FULLY NV (MODIFIKASI STATUS PAGE)
...
local function blinkTo(pos)
	...
	while fullyNVRunning and (hrp.Position - pos).Magnitude > 5 do
...
-- Fungsi Tween Slow untuk apart
local function tweenToApart(cf)
	local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
	if not hrp then return end
	local tween = TweenService:Create(hrp, TweenInfo.new(2.0, Enum.EasingStyle.Linear), {CFrame = cf})
	tween:Play()
	tween.Completed:Wait()
	if hrp and (hrp.Position - cf.Position).Magnitude > 3 then
		hrp.CFrame = cf
	end
end
...
local function cookAtApart()
	local coords = APART_COORDS[fullySelectedApart]
...
local function fullyMainLoop()
	while fullyNVRunning do
		...
		if not blinkTo(APART_ENTRY[fullySelectedApart]) then break end
		hideLoading()
...
statusFrame.Position = UDim2.new(0, 12, 0, 280)
...
statFrame.Position = UDim2.new(0, 12, 0, 335)
...
waterRow.Position = UDim2.new(0, 12, 0, 450)
local waterLabelF = Instance.new("TextLabel", waterRow)
...
local waterValueF = Instance.new("TextLabel", waterRow)
...
waterValueF.TextColor3 = C.blue

local sugarRow = card(fullyScroll, 25, 0)
sugarRow.Position = UDim2.new(0, 12, 0, 480)
local sugarLabelF = Instance.new("TextLabel", sugarRow)
...
local sugarValueF = Instance.new("TextLabel", sugarRow)
...
sugarValueF.TextColor3 = C.blue

local gelatinRow = card(fullyScroll, 25, 0)
gelatinRow.Position = UDim2.new(0, 12, 0, 510)
local gelatinLabelF = Instance.new("TextLabel", gelatinRow)
...
local gelatinValueF = Instance.new("TextLabel", gelatinRow)
...
gelatinValueF.TextColor3 = C.blue

local emptyRow = card(fullyScroll, 25, 0)
emptyRow.Position = UDim2.new(0, 12, 0, 540)
local emptyLabelF = Instance.new("TextLabel", emptyRow)
...
local emptyValueF = Instance.new("TextLabel", emptyRow)
...
emptyValueF.TextColor3 = C.blue

-- Tombol Start/Stop
local btnFrame = Instance.new("Frame", fullyScroll)
btnFrame.Size = UDim2.new(1, 0, 0, 50)
btnFrame.Position = UDim2.new(0, 0, 0, 575)
btnFrame.BackgroundTransparency = 1

fullyStartBtn = Instance.new("TextButton", btnFrame)
...
fullyStopBtn = Instance.new("TextButton", btnFrame)
