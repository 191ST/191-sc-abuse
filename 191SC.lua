...
-- (PAGE UNDERPOT DIGANTI DENGAN FULLY_NV)
...
local tabDefs = {
	...
	{label = "FULLY_NV", order = 7},  -- UNDERPOT DIGANTI JADI FULLY_NV
}
...
-- ============================================================
-- UI COMPONENT BUILDERS (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- FARM PAGE (SAMA PERSIS DENGAN ASLI)
-- ============================================================
local fp = pages["FARM"]
...
-- ============================================================
-- AUTO PAGE (SAMA PERSIS DENGAN ASLI)
-- ============================================================
local ap = pages["AUTO"]
...
-- ============================================================
-- STATUS PAGE (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- TELEPORT PAGE (SAMA PERSIS DENGAN ASLI)
-- ============================================================
local tp = pages["TP"]
...
-- ============================================================
-- ESP PAGE (SAMA PERSIS DENGAN ASLI)
-- ============================================================
local ep = pages["ESP"]
...
-- ============================================================
-- RESPAWN PAGE (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- PAGE FULLY_NV (MENGANTIKAN UNDERPOT)
-- ============================================================
...
if fullyNVPage then
	...
	-- Dropdown Apart
	local apartCard = card(fullyNVPage, 50, 1)
	...
	apartBtn.MouseButton1Click:Connect(function()
		...
		apartMenu = Instance.new("Frame", fullyNVPage)
	...
	-- Dropdown Pot
	local potCard = card(fullyNVPage, 50, 2)
	...
	potBtn.MouseButton1Click:Connect(function()
		...
		potMenu = Instance.new("Frame", fullyNVPage)
	...
	-- Button START / STOP
	local startBtn = makeActionBtn(fullyNVPage, "▶ START FULLY", C.green, 3)
	local stopBtn = makeActionBtn(fullyNVPage, "■ STOP FULLY", C.red, 4)
	
	-- Status
	local statusCardNV = card(fullyNVPage, 36, 5)
	...
	local function runFullyNV()
		...
		if selectedApart == "APART CASINO 1" then
			if selectedPot == "KANAN" then stages = {a1t1,a1t2,a1t3,a1t4,a1t5,a1t6kanan,a1t7kanan}
			else stages = {a1t1,a1t2,a1t3,a1t4,a1t5,a1t6kiri,a1t7kiri} end
		elseif selectedApart == "APART CASINO 2" then
			if selectedPot == "KANAN" then stages = {a2t1,a2t2,a2t3,a2t4,a2t5kanan}
			else stages = {a2t1,a2t2,a2t3,a2t4,a2t5kiri} end
		elseif selectedApart == "APART CASINO 3" then
			if selectedPot == "KANAN" then stages = {a3t1,a3t2,a3t3,a3t4,a3t5,a3t6kanan,a3t7kanan}
			else stages = {a3t1,a3t2,a3t3,a3t4,a3t5,a3t6kiri,a3t7kiri} end
		elseif selectedApart == "APART CASINO 4" then
			if selectedPot == "KANAN" then stages = {a4t1,a4t2,a4t3,a4t4,a4t5kanan,a4t6kanan}
		...
		for idx, cf in ipairs(stages) do
			...
			spamE()
			statusLblNV.Text = "Status: Tahap " .. idx .. "/" .. #stages
			task.wait(0.2)
		end
		...
		statusLblNV.Text = "Status: Selesai"
		statusLblNV.TextColor3 = C.textMid
	end
	...
	stopBtn.MouseButton1Click:Connect(function()
		...
		startBtn.BackgroundColor3 = C.green
		startBtn.Text = "▶ START FULLY"
		statusLblNV.Text = "Status: Dihentikan"
		statusLblNV.TextColor3 = C.red
	end)
end

-- ============================================================
-- FARM LOGIC (SAMA PERSIS DENGAN ASLI)
-- ============================================================
local function cook()
	while running do
		if equip("Water") then
...
-- ============================================================
-- AUTO FARM (full v2 logic) (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- ANTI HIT + APPROACH LOGIC (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- STATUS LOOP (SAMA PERSIS DENGAN ASLI)
-- ============================================================
task.spawn(function()
...
-- ============================================================
-- MINIMIZE BUTTON (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- HIDE BUTTON (MOBILE) + KEYBIND Z (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
-- ============================================================
-- ESP SYSTEM (SAMA PERSIS DENGAN ASLI)
-- ============================================================
...
switchTab("FARM")
task.wait(0.3)
notify("ELIXIR 3.5", "Script berhasil diload! (UNDERPOT diganti FULLY_NV)", "success")
