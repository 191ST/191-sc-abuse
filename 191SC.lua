...
-- ============================================================
-- TAMBAHAN: FULLY NV PAGE (APART CASINO + JALAN + BLINK)
-- ============================================================
...
-- Main Fully NV Loop (pake cook dari script asli)
    ...
    while fullyNVRunning do
        for i, stage in ipairs(stages) do
            ...
            if stage.isCook then
                ...
                -- Panggil fungsi cook dari script asli (running = true dulu)
                local oldRunning = running
                running = true
                local cookSuccess = pcall(function()
                    cook() -- ini fungsi cook dari ELIXIR asli
                end)
                running = oldRunning
...
-- TAMBAH TAB FULLY NV
-- ============================================================
local function tambahTabFullyNV()
    -- Tambah ke tabDefs
    table.insert(tabDefs, {label = "FULLY NV", order = 8})
    
    -- Buat ulang sidebar (refresh)
    ...
    -- Info card
    local infoCard = card(fullyNVPage, 46, 2)
    local infoLbl = Instance.new("TextLabel", infoCard)
    infoLbl.Size = UDim2.new(1, -16, 1, 0)
    infoLbl.Position = UDim2.new(0, 8, 0, 0)
    infoLbl.BackgroundTransparency = 1
    infoLbl.Text = "Sistem jalan kaki + blink otomatis jika nabrak.\nSetiap tahap akan spam E 3x, lalu masak jika diperlukan."
    ...
    local apartCard = card(fullyNVPage, 80, 4)
    ...
    local potCard = card(fullyNVPage, 60, 6)
    
    local potKananBtn = Instance.new("TextButton", potCard)
    ...
    local potKiriBtn = Instance.new("TextButton", potCard)
    ...
    -- Buttons Start/Stop
    local btnCard = card(fullyNVPage, 50, 9)
    local startFullyBtn = Instance.new("TextButton", btnCard)
    ...
    local stopFullyBtn = Instance.new("TextButton", btnCard)
