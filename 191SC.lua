...
-- ============================================================
-- TAMBAHAN: FULLY NV PAGE (TEMPEL DI SINI, SEBELUM STARTUP)
-- ============================================================

local fullyExists = false
for _, def in ipairs(tabDefs) do
    if def.label == "FULLY NV" then
        fullyExists = true
        break
    end
end
if not fullyExists then
    table.insert(tabDefs, {label = "FULLY NV", order = 8})
end
...
if not pages["FULLY NV"] then
    ...
    local layout = Instance.new("UIListLayout", newPage)
    layout.Padding = UDim.new(0, 7)
    layout.SortOrder = Enum.SortOrder.LayoutOrder
    local pad = Instance.new("UIPadding", newPage)
    pad.PaddingTop = UDim.new(0, 14)
    pad.PaddingLeft = UDim.new(0, 12)
    pad.PaddingRight = UDim.new(0, 12)
    pad.PaddingBottom = UDim.new(0, 14)
...
local function stepperRowNV(parent, y, lbl, minV, maxV, defV, unit)
...
local function cardNV(parent, h, order)
    local f = Instance.new("Frame", parent)
    f.Size = UDim2.new(1, 0, 0, h or 46)
    f.BackgroundColor3 = C.card
    f.BorderSizePixel = 0
...
local function sectionLabelNV(parent, text, order)
    local wrap = Instance.new("Frame", parent)
    wrap.Size = UDim2.new(1, 0, 0, 22)
    wrap.BackgroundTransparency = 1
    wrap.LayoutOrder = order or 0
...
local function makeStatusRowNV(parent, label, order)
    local f = cardNV(parent, 30, order)
    local lbl2 = Instance.new("TextLabel", f)
    lbl2.Position = UDim2.new(0, 12, 0, 0)
    lbl2.Size = UDim2.new(0.6, 0, 1, 0)
    lbl2.BackgroundTransparency = 1
...
sectionLabelNV(fullyScroll, "FULLY NV - AUTO APART CASINO", 1)

local apartCard = cardNV(fullyScroll, 80, 2)
...
local potCard = cardNV(fullyScroll, 80, 3)
...
local getBuyAmount = stepperRowNV(fullyScroll, 260, "Jumlah Set Bahan per Loop", 1, 50, 5, "x")

local statusCardNV = cardNV(fullyScroll, 50, 5)
...
local statCooked, _ = makeStatusRowNV(fullyScroll, "Total MS Dimasak", 8)
local statSold, _ = makeStatusRowNV(fullyScroll, "Total MS Terjual", 9)
