...
-- ============================================================
-- AUTO F SETIAP NEMBAK (HOOK REMOTE EVENT SHOOT)
-- ============================================================
local autoFOnShoot = false
...
-- Hook RemoteEvent.Shoot
local shootRemote = remotes and remotes:FindFirstChild("Shoot")
if shootRemote then
    local oldShoot = shootRemote.FireServer
    shootRemote.FireServer = function(self, ...)
        if autoFOnShoot then
            pressF()
        end
        return oldShoot(self, ...)
    end
    print("[AUTO F] Hook Shoot berhasil!")
else
    print("[AUTO F] Gagal menemukan RemoteEvent.Shoot")
end

-- Hook RemoteEvent.InflictTarget (buat damage)
local inflictRemote = remotes and remotes:FindFirstChild("InflictTarget")
if inflictRemote then
    local oldInflict = inflictRemote.FireServer
    inflictRemote.FireServer = function(self, ...)
        if autoFOnShoot then
            pressF()
        end
        return oldInflict(self, ...)
    end
    print("[AUTO F] Hook InflictTarget berhasil!")
end
...
sectionLabel(mspot, "HAPUS PART DI BAWAH", 1)
...
local findCookBtn = makeActionBtn(mspot, "CARI AMPE KETEMU", Color3.fromRGB(0, 100, 80), 5)
...
-- ============================================================
-- AUTO F SETIAP NEMBAK - TOGGLE DI SETTINGS
-- ============================================================
sectionLabel(settingsp, "AUTO F (SHOOT)", 8)

local autoFCard = card(settingsp, 50, 9)
local autoFTitle = Instance.new("TextLabel", autoFCard)
autoFTitle.Size = UDim2.new(0.6, 0, 1, 0)
autoFTitle.Position = UDim2.new(0, 12, 0, 0)
autoFTitle.BackgroundTransparency = 1
autoFTitle.Text = "🔫 Auto F Setiap Nembak"
autoFTitle.Font = Enum.Font.GothamSemibold
autoFTitle.TextSize = 12
autoFTitle.TextColor3 = C.text
autoFTitle.TextXAlignment = Enum.TextXAlignment.Left

local autoFToggleBtn = Instance.new("TextButton", autoFCard)
autoFToggleBtn.Size = UDim2.new(0, 80, 0, 32)
autoFToggleBtn.Position = UDim2.new(1, -92, 0.5, -16)
autoFToggleBtn.BackgroundColor3 = C.red
autoFToggleBtn.Text = "OFF"
autoFToggleBtn.Font = Enum.Font.GothamBold
autoFToggleBtn.TextSize = 12
autoFToggleBtn.TextColor3 = C.text
autoFToggleBtn.BorderSizePixel = 0
Instance.new("UICorner", autoFToggleBtn).CornerRadius = UDim.new(0, 6)

autoFToggleBtn.MouseButton1Click:Connect(function()
    autoFOnShoot = not autoFOnShoot
    if autoFOnShoot then
        autoFToggleBtn.Text = "ON"
        autoFToggleBtn.BackgroundColor3 = C.green
    else
        autoFToggleBtn.Text = "OFF"
        autoFToggleBtn.BackgroundColor3 = C.red
    end
...
local autoFInfo = Instance.new("TextLabel", autoFCard)
autoFInfo.Size = UDim2.new(1, -24, 0, 20)
autoFInfo.Position = UDim2.new(0, 12, 0, 28)
autoFInfo.BackgroundTransparency = 1
autoFInfo.Text = "⚠️ Setiap nembak akan otomatis tekan F"
autoFInfo.Font = Enum.Font.Gotham
autoFInfo.TextSize = 9
autoFInfo.TextColor3 = C.textDim
autoFInfo.TextXAlignment = Enum.TextXAlignment.Left
...
print("[191 STORE + AUTO F] Script loaded! - Auto F setiap nembak ada di tab SETTINGS")
