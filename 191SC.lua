-- ============================================================
-- ELIXIR 3.5 + FULLY NV (KETARIK + TURUN 4 STUDS)
...
local function makeStatusRow(parent, label, order)
	...
	val2.Text = "0"
	val2.Font = Enum.Font.GothamBold	val2.TextSize = 12
...
-- ============================================================
-- FULLY NV PAGE (KETARIK + TURUN 4 STUDS)
-- ============================================================

-- Kordinat Apart untuk Fully NV
local apartDataFully = {
    [1] = {
        name = "Apart Casino 1",
        stages = {
...
local selectedApartFully = 1
local selectedPotFully = "kanan"
local fullyTargetMS = 5
local fullyStatusLbl = nil

-- Fungsi Ketarik (pake CFrame lerp cepat)
local function tarikKeTarget(targetCF, duration)
    ...
    local endCF = targetCF
    local startTime = tick()
    
    while tick() - startTime < duration and fullyNVRunning do
        local alpha = (tick() - startTime) / duration
        local newCF = startCF:Lerp(endCF, alpha)
        hrp.CFrame = newCF
        task.wait()
    end
...
-- Fungsi turun blink (4 studs)
local function turunBlink(studs)
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if hrp then
...
-- Fungsi naik blink (opsional, bisa dipake kalo perlu)
local function naikBlink(studs)
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if hrp then
        hrp.CFrame = hrp.CFrame * CFrame.new(0, studs, 0)
...
-- Fungsi beli bahan sesuai target
local function beliBahan(target)
    notify("Fully NV", "Membeli bahan untuk " .. target .. " MS...", "info")
    for i = 1, target do
    ...
    notify("Fully NV", "Bahan selesai dibeli!", "success")
    return true
end

-- Fungsi jual semua MS
local function jualSemuaMS()
    notify("Fully NV", "Menjual semua Marshmallow...", "info")
    local bags = {"Small Marshmallow Bag", "Medium Marshmallow Bag", "Large Marshmallow Bag"}
    for _, bag in pairs(bags) do
    ...
    notify("Fully NV", "Penjualan selesai!", "success")
end

-- Fungsi get CFrame stage berdasarkan pot
...
-- Main Loop Fully NV
local function jalankanFullyNV(statusFunc)
    fullyNVRunning = true
    local apart = selectedApartFully
    local pot = selectedPotFully
    local stages = apartDataFully[apart].stages
    local target = fullyTargetMS
    
    -- Dapatkan posisi apart dari stage pertama
    local apartPos = getStageCF(stages[1], pot)
    if not apartPos then
        statusFunc("❌ Gagal dapat posisi apart!", C.red)
        fullyNVRunning = false
        return
    end
    ...
    while fullyNVRunning do
        -- 1. Beli bahan
        local success = beliBahan(target)
        if not success then break end
        
        -- 2. Ketarik ke NPC BUY (sebenernya kita udah di NPC, tapi biar aman)
        statusFunc("📍 Di NPC Store...", C.textMid)
        task.wait(0.5)
        
        -- 3. Turun 4 studs
        statusFunc("⬇️ Turun 4 studs...", C.accent)
        turunBlink(4)
        
        -- 4. Ketarik ke apart
        statusFunc("🏃 Ketarik ke apart...", C.accentGlow)
        ...
        for i, stage in ipairs(stages) do
            ...
            if not targetCF then
                statusFunc("❌ Stage " .. i .. " tidak ditemukan!", C.red)
                break
            end
            
            -- Ketarik ke stage
            statusFunc("📍 Stage " .. i .. " - Ketarik...", C.accentGlow)
            tarikKeTarget(targetCF, 0.8)
            
            -- Spam E 3x
            statusFunc("🎯 Stage " .. i .. " - Spam E...", C.green)
            spamE(3)
            task.wait(0.3)
            
            -- Jika stage ini adalah tempat masak (isCook = true)
            if stage.isCook then
                statusFunc("🍳 Memasak di stage " .. i .. "...", C.accentGlow)
                ...
                running = oldRunning
                statusFunc("✅ Masak selesai!", C.green)
                task.wait(1)
            end
        end
        
        -- 6. Turun 4 studs (opsional, sesuai permintaan)
        statusFunc("⬇️ Turun 4 studs...", C.accent)
        turunBlink(4)
        
        -- 7. Ketarik ke NPC Jual
        statusFunc("🏃 Ketarik ke NPC jual...", C.accentGlow)
        tarikKeTarget(npcPos, 1.5)
        
        -- 8. Jual semua MS
        statusFunc("💰 Menjual MS...", C.green)
        jualSemuaMS()
        
        statusFunc("🔄 Selesai 1 loop, mengulang...", C.accent)
        task.wait(1)
    end
    
    fullyNVRunning = false
    statusFunc("✅ Fully NV dihentikan", C.orange)
end

-- Buat page Fully NV
local fullyPage = pages["FULLY NV"]

sectionLabel(fullyPage, "AUTO FULLY NV (KETARIK)", 1)

-- Info card
local infoCardFully = card(fullyPage, 46, 2)
local infoLblFully = Instance.new("TextLabel", infoCardFully)
infoLblFully.Size = UDim2.new(1, -16, 1, 0)
infoLblFully.Position = UDim2.new(0, 8, 0, 0)
infoLblFully.BackgroundTransparency = 1
infoLblFully.Text = "Sistem KETARIK (cepat) + turun 4 studs sebelum ke apart.\nBeli bahan → turun → ketarik ke apart → ketarik tiap stage → spam E → cook → jual → loop"
infoLblFully.Font = Enum.Font.Gotham
infoLblFully.TextSize = 11
infoLblFully.TextColor3 = C.textMid
infoLblFully.TextWrapped = true

-- Slider Target MS
sectionLabel(fullyPage, "TARGET MASAK", 3)
local targetSliderWrap, targetValLbl = makeSlider(fullyPage, "Target MS per loop", 1, 50, 5, 4, function(v)
    fullyTargetMS = v
end)

-- Pilih Apart
sectionLabel(fullyPage, "PILIH APART CASINO", 5)
local apartCardFully = card(fullyPage, 80, 6)

local apartButtonsFully = {}
local apartNamesFully = {"Apart 1", "Apart 2", "Apart 3", "Apart 4"}
for i, name in ipairs(apartNamesFully) do
    local btn = Instance.new("TextButton", apartCardFully)
    ...
    btn.MouseButton1Click:Connect(function()
        selectedApartFully = i
        for _, b in pairs(apartButtonsFully) do
    ...
    table.insert(apartButtonsFully, btn)
end

-- Pilih Pot
sectionLabel(fullyPage, "PILIH POT (KANAN/KIRI)", 7)
local potCardFully = card(fullyPage, 60, 8)

local potKananBtnFully = Instance.new("TextButton", potCardFully)
potKananBtnFully.Size = UDim2.new(0.45, -5, 0, 36)
potKananBtnFully.Position = UDim2.new(0.03, 0, 0.2, 0)
potKananBtnFully.BackgroundColor3 = C.accent
potKananBtnFully.Text = "POT KANAN"
potKananBtnFully.Font = Enum.Font.GothamBold
potKananBtnFully.TextSize = 12
potKananBtnFully.TextColor3 = C.text
potKananBtnFully.BorderSizePixel = 0
Instance.new("UICorner", potKananBtnFully).CornerRadius = UDim.new(0, 8)

local potKiriBtnFully = Instance.new("TextButton", potCardFully)
potKiriBtnFully.Size = UDim2.new(0.45, -5, 0, 36)
potKiriBtnFully.Position = UDim2.new(0.52, 0, 0.2, 0)
potKiriBtnFully.BackgroundColor3 = C.card
potKiriBtnFully.Text = "POT KIRI"
potKiriBtnFully.Font = Enum.Font.GothamBold
potKiriBtnFully.TextSize = 12
potKiriBtnFully.TextColor3 = C.text
potKiriBtnFully.BorderSizePixel = 0
Instance.new("UICorner", potKiriBtnFully).CornerRadius = UDim.new(0, 8)

potKananBtnFully.MouseButton1Click:Connect(function()
    selectedPotFully = "kanan"
    TweenService:Create(potKananBtnFully, TweenInfo.new(0.15), {BackgroundColor3 = C.accent}):Play()
    TweenService:Create(potKiriBtnFully, TweenInfo.new(0.15), {BackgroundColor3 = C.card}):Play()
end)

potKiriBtnFully.MouseButton1Click:Connect(function()
    selectedPotFully = "kiri"
    TweenService:Create(potKananBtnFully, TweenInfo.new(0.15), {BackgroundColor3 = C.card}):Play()
    TweenService:Create(potKiriBtnFully, TweenInfo.new(0.15), {BackgroundColor3 = C.accent}):Play()
end)

-- Status
sectionLabel(fullyPage, "STATUS", 9)
local statusCardFully = card(fullyPage, 36, 10)
fullyStatusLbl = Instance.new("TextLabel", statusCardFully)
...
fullyStatusLbl.TextSize = 12
fullyStatusLbl.TextColor3 = C.textMid
fullyStatusLbl.TextXAlignment = Enum.TextXAlignment.Center

-- Buttons Start/Stop
local btnCardFully = card(fullyPage, 50, 11)
local startFullyBtn = Instance.new("TextButton", btnCardFully)
...
local stopFullyBtn = Instance.new("TextButton", btnCardFully)
...
local function setFullyStatus(msg, col)
    if fullyStatusLbl then
        fullyStatusLbl.Text = msg
        fullyStatusLbl.TextColor3 = col or C.textMid
    end
...
startFullyBtn.MouseButton1Click:Connect(function()
    ...
    setFullyStatus("⚡ FULLY NV berjalan...", C.green)
    task.spawn(function()
        jalankanFullyNV(setFullyStatus)
    end)
end)
...
switchTab("FARM")
task.wait(0.3)
notify("ELIXIR 3.5", "Script berhasil diload! + FULLY NV (Ketarik)", "success")
